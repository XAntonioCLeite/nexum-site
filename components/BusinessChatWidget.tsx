...
const BusinessChatWidget: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 'welcome', role: 'model', text: 'Olá! Sou a IA da Nexum AI. 🤖 Como posso ajudar sua clínica a vender mais hoje?' }
  ]);
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const ai = new GoogleGenAI({ apiKey: 'AIzaSyCEv6TtBwvZx6JXZ_7BOOFirO5ZGUR4SXg' });
      ...
      // Split response by the "|||" delimiter
      const parts = responseText.split('|||').map(p => p.trim()).filter(p => p);

      setIsLoading(false); // Stop the initial "thinking" loader

      // Loop to add messages with a "typing" delay simulation
      for (let i = 0; i < parts.length; i++) {
        const text = parts[i];

        // For messages after the first one, simulate typing delay
        if (i > 0) {
           setIsLoading(true);
           // Delay based on text length (simulating reading/typing), max 2 seconds
        ...
        setMessages(prev => [
          ...prev,
          { id: (Date.now() + i).toString(), role: 'model', text: text }
        ]);
      }
      ...
      setMessages(prev => [
        ...prev,
        { id: (Date.now() + 1).toString(), role: 'model', text: "Desculpe, estou enfrentando uma instabilidade momentânea na rede neural. Tente novamente em instantes." }
      ]);
    }
  };
          ...
          >
            ...
            <div className="bg-[#1e293b] p-4 flex items-center justify-between border-b border-white/5">
              <div className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-tr from-deepBlue to-blue-600 flex items-center justify-center shadow-lg">
                  <Bot size={18} className="text-white" />
                </div>
                <div>
                  <h3 className="font-bold text-white text-sm">Nexum AI Assistant</h3>
                  <div className="flex items-center gap-1.5">
                    {/* Keep Mint for Status Dot Contrast */}
                    <span className="w-2 h-2 rounded-full bg-mint animate-pulse"></span>
                    <span className="text-[10px] text-mint font-medium uppercase tracking-wider">Online</span>
                  </div>
                </div>
              </div>
              <button 
            ...
            <div className="p-3 bg-[#1e293b] border-t border-white/5">
              ...
              <div className="text-center mt-2">
                 <p className="text-[10px] text-slate-500 flex items-center justify-center gap-1">
                   Powered by <span className="font-bold text-slate-400">Gemini 3 Flash</span>
                 </p>
              </div>
            </div>
          </motion.div>
      ...
      >
        ...
        {!isOpen && (
            <span className="absolute top-0 right-0 flex h-4 w-4">
              ...
              <span className="relative inline-flex rounded-full h-4 w-4 bg-mint border-2 border-black"></span>
            </span>
        )}
      </motion.button>
    </div>
